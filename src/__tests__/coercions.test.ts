  ...
  it('returns the same value as unsafeFromNumber()', () => {
    fc.assert(
      fc.property(
        ...
        fc.currency(),
        fc.oneof(fc.precision(4), fc.constant(undefined)),
        fc.roundingFunction(),
        (number, currency, precision, roundingFunction) => {
          expect(
            fromNumber(number, currency, precision, roundingFunction),
          ).toBeIdenticalToMonetaryValue(
            unsafeFromNumber(number, currency, precision, roundingFunction),
          );
        },
      ),
    );
...
describe('unsafeFromNumber()', () => {
  ...
  it('returns a monetary value whose precision is the provided one, or zero if not provided', () => {
    fc.assert(
      fc.property(
        fc.number(),
        fc.currency(),
